[Ir para o conteúdo](eventos-kafka-spring.md#apache-kafka-com-spring)
...
O SpringBoot nos permite realiar toda a configuração do Kafka de forma declarativa, sem a necessidade de escrita de nenhuma linha de código. Para isso, o arquivo application.properties ou aplication.yaml deve ser alterado para incluir as informações de configuração do cluster Kafka.
...
O Spring-kafka já fornece um template pronto para o envio de mensagens para tópicos Kafka.

O KafkaTemplate é a classe que faz o envio de mensagens para os tópicos, ela utiliza as configurações de conexões informadas declarativamente no arquivo application.properties[yaml], sendo necessário informar apenas o tópico para onde a mensagem será enviada.
...
A criação de consumidores de tópicos do Kafka com Spring é bastante simples e segue o modelo adotado para a conexão em filas MQ. Deve-se apenas incluir a anotação “@KafkaListener”. Essa anotação indicará ao framework as informações a respeito do tópico, consumer groups, etc.
...
| --- | --- | --- | --- |
| 14/12/2021 | 1 | Criação do documento. | Suart02 |
| 10/01/2022 | 2 |  | Suart02 |

[Anterior Kafka com Quarkus](eventos-kafka-quarkus.md) [Próximo Transmissão de Arquivos](infraestrutura-transmissao_arquivos.md)

2020 © Caixa Econômica Federal. Todos os direitos reservados. - [Preferências de cookies](eventos-kafka-spring.md#__consent)
