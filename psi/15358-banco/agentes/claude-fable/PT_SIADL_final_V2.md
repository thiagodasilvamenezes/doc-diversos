...
A primeira leitura da DDL implementada revelou um padrão que mudou a natureza deste diagnóstico: os problemas encontrados não são desvios pontuais de tuning, mas **violações objetivas de padrões corporativos publicados** — datatypes em desacordo com o permitido para SQL Server, tabelas bilionárias sem o particionamento que a norma manda avaliar, ausência da compactação que é o padrão físico institucional e nomenclatura que não passaria pelo pré-validador. Diante disso, adoto deliberadamente a **postura de auditoria de conformidade**: cada achado é confrontado com o padrão corporativo aplicável, com **causa → impacto → direcionamento**. Essa abordagem tem duas vantagens práticas — tira a discussão do campo da opinião ("boa prática" vira "descumprimento de item normativo", com régua objetiva) e já estrutura o plano de correção na mesma ordem de prioridade que a banca de governança cobraria.
...
| Padronização          | Nomenclatura (TE074/Anexo II), domínios, glossário, dicionário de dados                                | Objetos físicos, filegroups, padrões de índice/constraint                                                                                                                                                                                         | Glossário e padrões divulgados aos squads; dúvidas de nomenclatura resolvidas no refinamento, antes do código.     |
| Qualidade e significado | Regras de negócio, qualidade e linhagem do dado, catálogo, classificação da informação (OR016/LGPD) | Integridade física implantada (FKs, CHECKs, unique)                                                                                                                                                                                                 | Histórias devem trazer regras de dados.                                                                               |
| Performance             | Informa volumetria, crescimento e padrões de acesso no modelo                                            | Índices, particionamento, compressão, estatísticas, tuning, planos                                                                                                                                                                                | Testes de carga e Query Store.                                                                                         |
...
| Operação              | Impactos negociais, apoio ao PO e ao gestor da informação                                               | Capacidade, backup/restore, alta disponibilidade, segurança física/lógica, monitoramento, execução controlada em produção                                                                                                                     | Comunicação de janelas e implantações ao time; acompanhamento conjunto pós-implantação com critérios go/no-go. |
...
Estratégia em duas frentes — **proativa** (agir antes da degradação) e **corretiva tempestiva** (responder rápido e formalmente ao desvio) — cobrindo as cinco dimensões exigidas.
...
| ------------------------------------ | ---------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Timeout em extrato por conta         | Índice emergencial por conta/período (autonomia formal do DBA)             | Reavaliar estratégia de índices e particionamento no comitê      |
| Picos de CPU por scan em histórico  | Análise de plano, estatística manual, forçamento de plano via Query Store | Mover consulta analítica para réplica/partição fria/columnstore |
| Bloqueios leitura×escrita           | Verificar transações longas e isolamento; avaliar RCSI                     | Revisar padrões de acesso e desenho transacional da aplicação    |
| Crescimento explosivo do histórico  | Bloquear fonte de duplicidade; medir geração; retenção emergencial       | Implantar ciclo de vida completo e rolling window                   |
| Dados inconsistentes em canal/status | Trava de domínio (FK/CHECK) e rotina de saneamento                          | Ciclo de qualificação de dados (TE169) com causa raiz             |
