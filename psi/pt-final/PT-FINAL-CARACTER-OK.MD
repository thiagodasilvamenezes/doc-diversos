...
O tratamento de emergências segue um *runbook* com SLA rigoroso de até 60 minutos. A `hotfix/<ID-Incidente>` nasce da `main` com a correção estritamente necessária. Para garantir a **proibição de refatorações de escopo**, institui-se uma governança dupla:
...
* **Versionamento Semântico Imutável (SemVer)**: Abolição de sufixos instáveis (`SNAPSHOT`). Toda liberação produtiva recebe uma marcação estática (*Git Tag* `vX.Y.Z`) atrelada ao hash SHA-1 do *commit*, garantindo paridade absoluta com a Requisição de Mudança formal (submetida com os 10 dias regulamentares de antecedência).
